...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            if (last.getTile().isEmpty()
                || last.getTile().getOccupant().getOwner()
                    == previousLastClicked.getTile().getOccupant().getOwner()
                || !((MilitaryUnit) previousLastClicked.getTile()
                        .getOccupant()).getCanAttack()
                || !GridFX.adjacent(previousLastClicked.getTile(),
                        last.getTile())) {
                error.play();
                Alert alert = new Alert(AlertType.WARNING);
                alert.setTitle("Warning Dialog");
                alert.setHeaderText("This action cannot be performed!");
                alert.setContentText("You can't attack this tile. "
                    + " Press OK to continue");
                alert.showAndWait();
            } else {
                attack(last);
                AudioClip attacked =
                    new AudioClip("File:./src/main/java/view/music/Sword.mp3");
                attacked.play();
            }
            ...
            if (move(last)) {
                AudioClip moved =
                    new AudioClip("File:./src/main/java/view/music/Run.mp3");
                moved.play();
                lastClicked = last;
            } else {
                error.play();
                ...
                alert.setTitle("Warning Dialog");
                alert.setHeaderText("This action cannot be performed!");
                alert.setContentText("You can't move "
                    + "to this tile. Press OK to continue");
                alert.showAndWait();
            }
        }
        //update lastClicked
    ...
    public static void ai() {
        ...
        for (int r = 0; r < GridFX.getMapLength(); r++) {
            for (int c = 0; c < GridFX.getMapLength(); c++) {
        ...
        if (banditHideout != null) {
            if (rand.nextInt(100) < GridFX.getMapLength()) {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < GridFX.getMapLength() && newCol >= 0
                && newCol < GridFX.getMapLength()) {
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < GridFX.getMapLength() && c > 0
            && c < GridFX.getMapLength())) {
